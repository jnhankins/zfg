...
public final class ParseExpr {
  ...
  public static final Node visitLiteralExpr(final Parser parser, final LiteralExprContext ctx) {
    ...
    return switch (ctx.lit.getType()) {
      case BitLit -> switch (zfg.old.lang3.val.Parser.parseBit(str)) {
        case null -> {
      ...
      case IntLit -> {
        ...
        yield switch (zfg.old.lang3.val.Parser.parseInt(str, hasNegativeSignPrefix)) {
          case null -> {
      ...
      case FltLit -> switch (zfg.old.lang3.val.Parser.parseFlt(str)) {
        case null -> {
